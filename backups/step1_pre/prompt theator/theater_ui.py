...
class PromptTheatorUI:
    def __init__(self, root):
        ...
        # Engine Control
        tk.Label(left_panel, text="Backend Engine:", bg=WIN_GREY, font=COMIC_FONT).pack(anchor="w", pady=(10,0))
        self.engine_var = tk.StringVar(value="Ollama (Standard)")
        self.engine_menu = ttk.Combobox(left_panel, textvariable=self.engine_var, values=["Ollama (Standard)", "GGML Native (CDLSS)"], state="readonly")
        self.engine_menu.pack(fill="x", padx=5)
        ...
        self.tokens_var = tk.IntVar(value=0)
        ...
        tk.Label(left_panel, text="Storm Intensity (Concurrency):", bg=WIN_GREY, font=COMIC_FONT).pack(anchor="w", pady=(10,0))
        ...
        tk.Label(left_panel, text="Inference Threads (CPU):", bg=WIN_GREY, font=COMIC_FONT).pack(anchor="w", pady=(10,0))
        self.threads_var = tk.IntVar(value=14)
        self.threads_menu = ttk.Combobox(left_panel, textvariable=self.threads_var, 
                                        values=[1, 2, 4, 6, 8, 12, 14, 16, 24, 32], state="readonly")
        self.threads_menu.pack(fill="x", padx=5)
        ...
        self.btn_deepmap = tk.Button(output_header, text=" DEEP MAP ", command=self.execute_deep_map,
                                     font=COMIC_FONT, bg="#008080", fg="white", bd=2, relief="raised", padx=5)
        self.btn_deepmap.pack(side="right", padx=10)
        ...
        try:
            ...
            from research.engine_ggml import GGMLEngineV1
            from bio_log_manager import BioLogManager
            from storm_logic import StormLogic
            from persona_manager import PersonaManager
            
            self.engine_v1 = SimpleEngineV1()
            self.engine_ggml = GGMLEngineV1()
            self.engine = self.engine_v1
            self.mascot_engine = self.engine_v1 # Mascot uses its own instance
            self.logic = StormLogic()
            base_dir = os.path.dirname(os.path.abspath(__file__))
            self.pm = PersonaManager(characters_dir=os.path.join(base_dir, "characters"))
            self.bio_log = BioLogManager(
                context_file=os.path.join(base_dir, "active_context.json"),
                archive_file=os.path.join(base_dir, "biolog.ndjson"),
                stash_file=os.path.join(base_dir, "last_stash.json")
            )
    ...
    def on_engine_change(self, event=None):
        if self.engine_var.get() == "GGML Native (CDLSS)":
            self.engine = self.engine_ggml
        else:
            self.engine = self.engine_v1
        self.refresh_models()
    ...
    def execute_deep_map(self):
        """Starts a cumulative Deep Map run."""
        model = self.model_var.get()
        n = self.traj_var.get()
        
        self.log_sys(f"--- DEEP MAP INITIATED: {model} (N={n}) ---")
        self._force_collapse.clear()
        self.btn_execute.config(state="disabled", text=" BUSY ")
        self.btn_deepmap.config(state="disabled", text=" MAPPING... ")
        self.btn_collapse.config(state="normal")
        self.progress.start(10)
        
        # Worker thread for topography
        threading.Thread(target=self.run_deep_map_thread, args=(model, n), daemon=True).start()

    def run_deep_map_thread(self, model, n):
        try:
            from research import cdlss_topography_ollama
            
            def ui_log(msg):
                self.root.after(0, lambda: self.log_sys(msg))
                
            def ui_prog(curr, total):
                self.root.after(0, lambda: self.progress.configure(value=curr, maximum=total))

            img_path = cdlss_topography_ollama.run_deep_map(
                model_name=model,
                num_trajectories=n,
                log_callback=ui_log,
                progress_callback=ui_prog,
                stop_event=self._force_collapse,
                num_thread=self.threads_var.get()
            )
            
            if img_path and os.path.exists(img_path):
                self.root.after(0, lambda: self.log_sys(f"Deep Map Complete: {os.path.basename(img_path)}"))
                # Open the image on Windows
                os.startfile(img_path)
            else:
                self.root.after(0, lambda: self.log_sys("Deep Map finished with no output."))
                
        except Exception as e:
            self.root.after(0, lambda: self.log_sys(f"DEEP MAP ERROR: {e}"))
        finally:
            self.root.after(0, self.finish_deep_map)

    def finish_deep_map(self):
        self.progress.stop()
        self.progress.configure(value=0)
        self.btn_execute.config(state="normal", text=" EXECUTE ")
        self.btn_deepmap.config(state="normal", text=" DEEP MAP ")
        self.btn_collapse.config(state="disabled")
        self.log_sys("Ready for next operation.")
    ...
    def run_engine(self, prompt):
        ...
        try:
            ...
            if recursive:
                p_label = current_persona if current_persona != "None" else "Default Persona"
                self.log_sys(f"STAGE 1/2: Recursive Refinement via {p_label}...")
                ...
                # Log Stage 1 to cumulative map
                self.update_cumulative_map(main_model, prompt, refine_trajs)

            # --- STAGE 2: Final Storm & Dual Collapse ---
            p_label = current_persona if current_persona != "None" else "Default Persona"
            self.log_sys(f"STAGE 2: Final Theatrical Storm ({p_label})...")
            ...
            def run_single_path(idx):
                ...
                if self.engine_var.get() == "GGML Native (CDLSS)":
                    res = self.engine.generate(
                        ...
                        num_predict=tokens, 
                        num_thread=self.threads_var.get(),
                        cdlss_trajectories=n,
                        cdlss_dcx=float(self.dcx_high.get())
                    )
                else:
                    res = self.engine.generate(full_prompt, temperature=temp, num_predict=tokens, num_thread=self.threads_var.get())
                completed += 1
            ...
            storm_result = self.logic.analyze_storm(
                trajectories, model_names, engine=self.engine, high_thresh=h_thresh
            )
            ...
            if self.toggle_audit.get():
                ...
                # Always save per-session audit
                self.save_storm_audit(audit_data)
                
                # Also update cumulative model map (Final Stage)
                log_prompt = current_prompt if recursive else prompt
                self.update_cumulative_map(main_model, log_prompt, trajectories)
    ...
    def save_storm_audit(self, data):
        """Saves deep diagnostic storm data to JSON with robust serialization."""
        base_dir = os.path.dirname(os.path.abspath(__file__))
        folder = os.path.join(base_dir, "storms")
        ...
        try:
            # Use json.dumps with custom encoder + explicit flush/sync
            content = json.dumps(data, cls=NumpyEncoder, indent=4)
            with open(path, "w", encoding="utf-8") as f:
                f.write(content)
                f.flush()
                os.fsync(f.fileno())
            self.log_sys(f"Deep Audit saved: {filename} ({len(content)} bytes)")
        except Exception as e:
            self.log_sys(f"AUDIT FAILED: {type(e).__name__}: {e}")

    def update_cumulative_map(self, model_name, prompt, new_trajs):
        """Appends new trajectories to the cumulative model history JSON."""
        base_dir = os.path.dirname(os.path.abspath(__file__))
        folder = os.path.join(base_dir, "maps")
        if not os.path.exists(folder): os.makedirs(folder)
        
        from research.cdlss_topography_ollama import slugify
        model_slug = slugify(model_name)
        path = os.path.join(folder, f"deep_map_{model_slug}.json")
        ...
        try:
            ...
            # Ensure responses structure is ready
            if "responses" not in data: data["responses"] = {}
            if prompt not in data["responses"]: data["responses"][prompt] = []
            
            # Append new trajectories
            data["responses"][prompt].extend(new_trajs)
            data["total_trajectories"] = sum(len(v) for v in data["responses"].values())
            ...
            with open(path, "w", encoding="utf-8") as f:
                ...
                f.flush()
                os.fsync(f.fileno())
            self.log_sys(f"Map Growth: {model_name} total={data['total_trajectories']}")
        except Exception as e:
            self.log_sys(f"MAP SYNC FAILED: {e}")
